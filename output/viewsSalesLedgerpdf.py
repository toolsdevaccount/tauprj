...
from myapp.output import salesledgerfunction, viewsGetTaxRateFunction, GetBalancefunction, viewsGetDateFunction
# 検索機能のために追加
...
def pdf(request, TargetMonth, element_From, element_To):
    try:
        ...
        # 文字列を日付に変換する
        search_date = viewsGetDateFunction.conversionledger(TargetMonth)
...
def make(search_date, element_From, element_To, response):
    ...
    for i in range(counter):
        ...
        if dt_Prev[0]!=0 or dt_Prev[6]!=0:
            salesledgerfunction.printstring(pdf_canvas, dt, dt_Prev, dt_Detail, search_date, dt_Taxrate)
            result=0
            PrCnt=1
