    ...
    {
        //桁区切りして配置（仕入単価）
        UnitPrice = Number(McdUnitPrice(obj)).toLocaleString(undefined, { minimumFractionDigits: 2,maximumFractionDigits: 2 });
    ...
    {
        //桁区切りして配置（仕入単価）
        var num = detail(obj).querySelectorAll('.McdDtlPrice').length;

        for (i=0; i < num; i++) {
            var item = detail(obj).querySelectorAll('.McdDtlPrice')[i].value;
            if(isNaN(item)==false){
                var DtlPrice = Number(item).toLocaleString(undefined, { minimumFractionDigits: 2,maximumFractionDigits: 2 });
            }else{
                var DtlPrice = item.toLocaleString(undefined, { minimumFractionDigits: 2,maximumFractionDigits: 2 });
            }

            detail(obj).querySelectorAll('.McdDtlPrice')[i].value = DtlPrice;
        }
